...
class ResidualAlexNet(nn.Module):
    def __init__(self, num_classes=10):
        ...
        self.relu = nn.PReLU()  # Replace LeakyReLU with PReLU
        ...
        self.classifier = nn.Sequential(
            ...
            nn.PReLU(),  # Replace LeakyReLU with PReLU
            nn.Dropout(),
            nn.Linear(4096, 4096),
            nn.PReLU(),  # Replace LeakyReLU with PReLU
...
# 实例化模型并打印展示
# 实例化模型并打印展示
net = ResidualAlexNet(10).to(DEVICE)
print(net)
print(net(torch.randn([1,1,64,64]).to(DEVICE)))
...
# 保存模型
torch.save(model.state_dict(), 'ResidualAlexNet-PRelu.pth')

# 加载模型
model.load_state_dict(torch.load('ResidualAlexNet-PRelu.pth'))
