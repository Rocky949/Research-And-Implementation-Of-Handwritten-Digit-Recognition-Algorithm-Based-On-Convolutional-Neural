...
# 保存模型
torch.save(model.state_dict(), 'ResidualAlexNet.pth')

# 加载模型
model.load_state_dict(torch.load('ResidualAlexNet.pth'))
...
_, predictions = model.forward(features[8].unsqueeze(0).to(DEVICE))
